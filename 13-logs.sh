...
#$ echo 14-logs.sh | cut -d "." -f1 ---we get14-logs

mkdir -p $LOGS_FOLDER  # -p if folder not created it creates otherwise no changes happen
echo "script started executing at: $(date)" | tee -a $LOG_FILE  ## &>> tee -a
...
VALIDATE(){
    ...
    then
        ...
        echo -e "give $R ERROR $2 is not installed $N" t| tee -a $LOG_FILE
        exit 1
    fi
}
